...
export async function getProjectMembers(projectId) {
  ...
  const ownerMember = {
    ...
    avatar: project.owner.name ? project.owner.name.substring(0, 2).toUpperCase() : "OW", // Simple mock avatar
  };

  // 3. Format other members
  const otherMembers = project.members.map((m) => ({
    id: m.userId,
    name: m.user.name,
    email: m.user.email,
    role: m.role,
    joinedAt: m.joinedAt,
    avatar: m.user.name ? m.user.name.substring(0, 2).toUpperCase() : "ME",
  }));
